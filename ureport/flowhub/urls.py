...
urlpatterns = [
    ...
    url(
        r"^(?P<flow>[0-9]+)/download/$",
        views.DownloadView.as_view(),
        name="flowhub.flow_download",
    ),
    url(
        r"^(?P<flow>[0-9]+)/stars/$", views.StarView.as_view(), name="flowhub.flow_star"
    ),
    url(
        r"^(?P<flow>[0-9]+)/update/$",
        views.EditView.as_view(),
        name="flowhub.flow_update",
    ),
    url(
        r"^(?P<flow>[0-9]+)/delete/$",
        views.DeleteView.as_view(),
        name="flowhub.flow_delete",
    ),
    url(
        r"^(?P<flow>[0-9]+)/info/$", views.InfoView.as_view(), name="flowhub.flow_info"
    ),
]
